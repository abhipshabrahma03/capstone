...
void cmd_cd(const string &path) {
    try {
        fs::current_path(path);
@@ -50,173 +45,28 @@ void cmd_touch(const string &file) {
    f.close();
}
...
void cmd_pwd() {
    cout << fs::current_path().string() << "\n";
@@ -50,6 +45,7 @@ void cmd_touch(const string &file) {
    f.close();
}
...
void cmd_cp(const string &src, const string &dest) {
    try {
        fs::copy(src, dest, fs::copy_options::overwrite_existing);
@@ -70,7 +66,7 @@ void cmd_mv(const string &src, const string &dest) {
...
void cmd_rm(const string &target) {
    try {
        ...
        if (fs::remove(target))
            ...
            cout << "rm: no such file or directory\n";
@@ -92,104 +88,22 @@ void cmd_info(const string &file) {
        time_t cftime = chrono::system_clock::to_time_t(sctp);
        cout << "File: " << file << "\n";
        cout << "Type: " << (fs::is_directory(file) ? "Directory" : "File") << "\n";
        if (!fs::is_directory(file))
...
//day5
@@ -1,15 +1,14 @@//-------------------DAY 5----------------------//#include <bits/stdc++.h>#include <filesystem>#include <chrono>#include <cstdlib>using namespace std;namespace fs = std::filesystem;// ---------- Command History ----------// ---------- Command History ----------//vector<string> command_history;// ---------- Existing Commands ----------// ---------- Existing Commands ----------//void cmd_pwd() {    cout << fs::current_path().string() << "\n";}@@ -57,6 +56,7 @@ void cmd_cp(const string &src, const string &dest) {    } catch (...) {        cout << "cp: failed to copy file\n";    }    }void cmd_mv(const string &src, const string &dest) {
